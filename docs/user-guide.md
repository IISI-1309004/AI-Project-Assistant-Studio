...
| **Ollama** | 本機安裝 | **免費** | **推薦企業環境** |
| **遠端伺服器** | 公司內網 | ✅ 已有 | 連線至既有服務 |

> 💡 **企業推薦**：使用 Ollama（本機免費模型）或連線到公司已有的 Linux 伺服器

---

## 3. 安裝方式

### 3.1 Windows 一鍵安裝（包含 Docker）— 限自由安裝環境
...
> **注意**：此方案會安裝 Docker Desktop，公司限制環境請使用方案 3.3

---

### 3.2 Windows 本機模式（無 Docker）— 推薦企業環境
...
#### 步驟 4（推薦）：安裝 Ollama 本機 AI 模型

Ollama 是完全免費的獨立應用程式（無 Docker 依賴），可在本機離線運行 AI 模型。
...
**Q：可以離線使用嗎？**
✅ 是的，如果使用 Ollama 本機模型（不需網路）

**Q：可以連線到公司的 Linux 伺服器嗎？**
✅ 可以，在 `.env.local` 中修改 `AIPA_RUNTIME_URL` 指向公司伺服器（例如 `http://company-server:18080`）

---

### 3.3 社群伺服器模式（最推薦企業環境）

**適用於有公司 Linux 伺服器或雲端環境的企業**

此模式下 Windows 上只安裝 CLI，連線到已有的遠端 AIPA Runtime 服務。

#### 前置條件

- IT 部門已在公司 Linux 伺服器上部署 AIPA Runtime（使用 Linux 一鍵安裝腳本）
...
[System.Environment]::SetEnvironmentVariable("AIPA_RUNTIME_URL", "http://company-server:8080", "Machine")
...
```ini
AIPA_RUNTIME_URL=http://company-server:8080
AIPA_MODE=REMOTE
```
...
如果公司伺服器已部署好 AIPA，Windows 用戶只需要安裝 CLI 工具即可使用。

---

### 3.4 Docker Compose（Linux/macOS 手動部署）
