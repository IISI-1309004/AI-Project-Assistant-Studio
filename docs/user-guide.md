...
| 元件 | 需求 |
|------|------|
| **作業系統** | Ubuntu 22.04+、RHEL 8+、CentOS Stream 9、Windows 10/11 |
| **CPU** | 4 核心（建議 8 核心以上） |
| **記憶體** | 8 GB RAM（建議 16 GB） |
| **儲存空間** | 20 GB 可用空間（建議 50 GB） |
| **網路** | AI API 呼叫需要網際網路（Ollama 本地模式除外） |

### 軟體需求

| 軟體 | 版本 | 說明 |
|------|------|------|
| Docker | 24.0+ | Docker Compose 部署必要 |
| Docker Compose | 2.20+ | 服務編排 |
| Node.js | 20 LTS+ | CLI 工具執行環境 |
| Java JDK | 17 LTS+ | 如需從原始碼建置 |
| Python | 3.11+ | 如需從原始碼建置 |

### AI 供應商（至少設定一個）

| 供應商 | 需要 | 說明 |
|--------|------|------|
| Claude（Anthropic） | API Key | 推薦用於程式碼生成 |
| OpenAI | API Key | GPT-4 系列 |
| Google Gemini | API Key | Gemini 系列 |
| Ollama | 本地安裝 | 完全離線，不需 API Key |
...
# AI 供應商（至少設定一個）
CLAUDE_API_KEY=sk-ant-xxxxx               # Anthropic Claude
OPENAI_API_KEY=sk-xxxxx                   # OpenAI GPT-4
GEMINI_API_KEY=AIxxxxx                    # Google Gemini
OLLAMA_BASE_URL=http://host.docker.internal:11434  # Ollama 本地模型

# 資料庫（預設 SQLite，無需額外設定）
...
- **端口衝突**：預設使用 8080（Runtime）、80（Web UI）。如有衝突，在 `.env` 中修改 `RUNTIME_PORT` 和 `WEB_PORT`。
- **映像下載慢**：可在 `.env` 中設定 `DOCKER_REGISTRY` 使用自訂映像倉庫。

---

### 方式 B：Linux 伺服器一鍵安裝

適合 Ubuntu/RHEL/CentOS 伺服器部署。
...
安裝腳本會自動：
1. 檢查作業系統版本相容性
2. 安裝 Docker 和 Docker Compose（如尚未安裝）
3. 克隆程式庫到 `/opt/aipa-studio/`
4. 建立 systemd 服務（開機自動啟動）
5. 安裝 CLI 工具（全域可用）
6. 執行就緒健康檢查
...
# 啟動/停止服務
systemctl start aipa-studio
systemctl stop aipa-studio
...
腳本會自動：
- 檢查 Windows 版本
- 安裝 Docker Desktop（如尚未安裝）
- 設定 Docker Compose 服務
- 安裝 CLI 工具

3. 安裝完成後，重新開啟 PowerShell（更新 PATH）：
```powershell
aipa health
```
...
### 情境：為現有 Java Spring Boot 專案啟用 AIPA
...
查看規格內容（透過 Web Dashboard 或直接查看檔案）：
```bash
cat .ai-project/specs/sess_abc123/spec.md
```
