...
| 供應商 | 方式 | 成本 |
|--------|------|------|
| Claude（Anthropic） | API Key | 💳 付費 |
| OpenAI | API Key | 💳 付費 |
| Google Gemini | API Key | 💳 付費（含免費配額） |
| **Ollama** | 本地安裝 | **免費，完全離線** |

> 💡 **建議用 Ollama**：完全免費、無需外部 API 平台、所有企業資料留在本地。
...
腳本會自動：
- 檢查 Windows 版本和 WSL 2
- 安裝 Docker Desktop（如需要）
- 設定環境變數
- 啟動所有服務
- 安裝 CLI 工具全域可用

完成後重新開啟 PowerShell，驗證安裝：
...
如需新增 AI 供應商 API Key，在 **PowerShell** 執行：
...
# 或使用 Ollama 本地（無需 API Key）
# 先下載：https://ollama.ai
# 然後在 PowerShell 執行：ollama pull llama3.1:8b
```

重新啟動 Docker 以套用設定：

```powershell
docker compose -f 'C:\Program Files\aipa-studio\installer\docker\docker-compose.yml' restart
```

#### 常見問題

- **在哪裡可看到 Web Dashboard？** → 開啟瀏覽器訪問 `http://localhost`
- **Docker Desktop 安裝了但啟動失敗？** → 重新啟動電腦
- **需要停止服務？** → 執行 `docker compose -f 'C:\Program Files\aipa-studio\installer\docker\docker-compose.yml' down`

---

### 3.2 Docker Compose（Linux/macOS 手動部署）
